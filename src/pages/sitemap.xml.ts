 ...
 * ```
 */
async function getIndexablePages(): Promise<Page[]> {
  const pages: Page[] = [];

  const staticPages = Object.values(
  ...
  blogCollection.forEach(article => {
    const blogPage: Page = {
      lang: article.data.lang,
      slug: article.data.slug,
      excludeFromIndex: article.data.excludeFromIndex,
      lastmod: new Date(article.data.lastmod),
      type: PageType[article.data.type as keyof typeof PageType],
      slugTranslations: article.data.slugTranslations || {},
    };

    const blogUrl =
      article.data.lang === DEFAULT_LOCALE
        ? `blog/${article.data.slug}`
        : `blog/${article.data.lang}/${article.data.slug}`;

    if (blogPage.excludeFromIndex) {
      nonIndexablePages.push({ lang: blogPage.lang, slug: blogUrl });
    } else {
      indexablePages.push({ lang: blogPage.lang, slug: blogUrl });
      pages.push(blogPage);
    }
  });
  ...
  projectCollection.forEach(project => {
    const projectPage: Page = {
      ...
      excludeFromIndex: project.data.excludeFromIndex,
      lastmod: new Date(project.data.lastmod),
      type: PageType[project.data.type as keyof typeof PageType],
      slugTranslations: project.data.slugTranslations || {},
    };

    const projectUrl =
      project.data.lang === DEFAULT_LOCALE
        ? `project/${project.data.slug}`
        : `project/${project.data.lang}/${project.data.slug}`;

    if (projectPage.excludeFromIndex) {
      nonIndexablePages.push({ lang: projectPage.lang, slug: projectUrl });
    } else {
      indexablePages.push({ lang: projectPage.lang, slug: projectUrl });
 ...
 */
function isPageDirty(page: Page | undefined, localizedUrl: string): boolean {
  ...
  if (isDirty) {
    dirtyPages.push({ lang: page.lang, slug: localizedUrl });
    return true;
  }

  cleanPages.push({ lang: page.lang, slug: localizedUrl });
...
function getLocalizedURL(page: Page, lang: Locale): string {
  ...
  if (page.type == PageType.PROJECT) {
    prefix = "project/";
  }

  let url = `${prefix}${page.slug}`; // e.g. "/slashes/" or "/blog/first-article/"

  if (lang !== DEFAULT_LOCALE) {
    url = `/${lang}/${prefix}${page.slugTranslations[lang]}/`; // e.g. "/fr/barres-obliques/"
  }
...
function parsePagesToRoutes(pages: Page[]): RouteEntry[] {
  ...
  pages.forEach(page => {
    ...
    const localizedUrl = getLocalizedURL(page, lang);

    routes.push({
      url: localizedUrl,
      lang,
      lastmod: isPageDirty(page, localizedUrl) ? new Date() : lastmod,
      changefreq,
      priority,
      alternateVersions,
    });
...
export async function GET(): Promise<Response> {
  ...
  const pagesToIndex = await getIndexablePages();
